      ...
      className={`bg-slate-950 rounded-2xl border transition-all duration-300 overflow-hidden flex flex-col h-full ${
        isAssignedToMe 
          ...
          : isAppliedByMe 
            ...
            : 'border-slate-700 hover:border-emerald-100 shadow-xs hover:shadow-md'
      }`}
    >
      {/* Upper bar status */}
      <div className="px-5 pt-5 pb-3 flex items-center justify-between gap-2">
        ...
        <div className="text-right">
          <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">{t('job.card.price')}</p>
      ...
      <div className="px-5 pb-5 flex-1 flex flex-col">
        {/* Title & Specialties */}
        <div className="mb-4">
          <h3 className="font-sans font-bold text-slate-200 text-base leading-snug line-clamp-2 hover:text-emerald-700 transition-colors">
            {job.title}
          </h3>
          <p className="text-xs text-slate-500 mt-1.5 font-sans flex items-center gap-1.5 flex-wrap">
            <span>{t('job.card.client')}</span>
            <span className="font-semibold text-slate-300">{job.clientName}</span>
        ...
        <div className="flex items-center justify-between gap-2 mb-5 mt-auto">
          ...
          <div className="text-[10px] text-slate-500 font-medium">
            {job.applicants.length > 0 ? (
              ...
              <span className="text-slate-500 italic">{t('job.card.candidates.none')}</span>
            )}
          </div>
        </div>
